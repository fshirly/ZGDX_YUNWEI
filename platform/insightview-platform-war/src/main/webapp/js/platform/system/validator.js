		...
		{ 
			...
			notEmpty : {
				validator : function(value) {
					return value != '' && value.indexOf('请选择') == -1;
				},
				message : "该输入项为必输项"
			},
			// 验证中文
			...
			dlkpKpxSameName:{
				validator: function(value, param){
                    ...
                    if (!param[0]) {
                        $.ajax({
                                    ...
                                    success : function(data) {
                                        if (data) {
                                            flg = 1;
                                            rules.dlkpKpxSameName.message = "已存在同名考评项！";
                                        }
                                    }
                                });
                    }
                    if (flg == 0) {
                    	if(value.length > 500){
                    		rules.dlkpKpxSameName.message = "内容仅限500字以内！";
                    		return false;
                    	}
                    	return true;
                    } else {
			...
			columnLength:{
				validator: function(value, param){
					var rules = $.fn.validatebox.defaults.rules;
					rules.columnLength.message = "字数超过"+param+"个字！";
