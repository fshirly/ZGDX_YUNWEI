 ...
 */
function checkForm() {
	var checkControlAttr = new Array('ipt_deviceIP','ipt_userName','ipt_password');
	var checkMessagerAttr = new Array('请先选择设备！','请输入用户名！','请输入密码！');
			...
			.datagrid(
					{
						...
						fit : true,// 自动大小
						fitColumns : true,
						url : path + '/platform/sysVMIfCommunity/listSysVMIfCommunity',
						remoteSort : false,
						idField : 'fldId',
 ...
 * 加载管理域信息列表页面
 */
function loadDomianInfo(){
	var path=getRootName();
	var uri=path+"/platform/sysVMIfCommunity/toSelectDomainList";
...
function findDeviceInfo() {
	...
	var path = getRootName();
	var uri = path + "/platform/sysVMIfCommunity/findDeviceInfo";
	var ajax_param = {
		url : uri,
		type : "post",
		dateType : "json",
...
function getMoIDBymoName(moName) {
	var path = getRootName();
	var uri = path + "/platform/sysVMIfCommunity/getMoIdBymoName";
	var ajax_param = {
		url : uri,
		type : "post",
		datdType : "json",
...
function checkSyaAuthCommunity(flag, deviceIP) {
	if (checkIP()) {
		...
		var path = getRootName();
		var uri = path + "/platform/sysVMIfCommunity/checkDeviceIP";
		var ajax_param = {
				url : uri,
				type : "post",
				datdType : "json",
...
function doCheckUpdate() {
	if (checkIP()) {
		var path = getRootName();
		var uri = path + "/platform/sysVMIfCommunity/updateAuthCommunity";
		var ajax_param = {
			url : uri,
			type : "post",
			datdType : "json",
...
function initUpdateVal(id) {
	...
	var path = getRootName();
	var uri = path + "/platform/sysVMIfCommunity/findSysAuthCommunityByID";
	var ajax_param = {
		url : uri,
		type : "post",
		datdType : "json",
...
function doDel(id) {
	...
	$.messager.confirm("提示", "确定删除此条信息吗？", function(r) {
		if (r == true) {
			var path = getRootName();
			var uri = path + "/platform/sysVMIfCommunity/delAuthCommunity";
			var ajax_param = {
				url : uri,
				type : "post",
				datdType : "json",
		...
		$.messager
				.confirm(
						...
						function(r) {
							if (r == true) {
								var uri = path
										+ "/platform/sysVMIfCommunity/batchdelAuthCommunity?ids="
										+ ids;
								var ajax_param = {
									url : uri,
									type : "post",
									...
									success : function(data) {
										if (true == data || "true" == data) {
											$.messager.alert("提示", "信息批量删除成功！",
													"info");
											reloadTable();
										} else {
											$.messager.alert("提示", "信息批量删除失败！",
													"error");
											reloadTable();
										}
									}
...
function doCheckAdd() {
	if (checkIP()) {
		var path = getRootName();
		var uri = path + "/platform/sysVMIfCommunity/addAuthCommunity";
		var ajax_param = {
			url : uri,
			type : "post",
			datdType : "json",
