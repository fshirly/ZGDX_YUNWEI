...
(function(w){
	w.InputLinked = function(){
		//控件类型
		this.widgetType = 'InputLinked';
		...
		this.createWidget = function(data){
			var $input = $('<input readonly="true"/>');
			$input.attr('id',data.id).attr('widgetType',data.widgetType);
			return $input;
		};
	};
	//注册此控件类型
	ISV.pf.fd.base.FormWidgetFactory.register('InputLinked',w.InputLinked);
})(ISV.pf.fd.widget);