...
(function(w){
	w.SelectBothLink = function(){
		//控件类型
		this.widgetType = 'SelectBothLink';
...
//		};
		...
		this.createWidget = function(data){
			var $SelectBothLink = $('<input />');
			$SelectBothLink.attr('id',data.id).attr('widgetType',data.widgetType).attr('style','width:150px');;
			return $SelectBothLink;
		};
	...
	//注册此控件类型
	ISV.pf.fd.base.FormWidgetFactory.register('SelectBothLink',w.SelectBothLink);
})(ISV.pf.fd.widget);