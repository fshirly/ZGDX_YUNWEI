...
(function(w){
	w.SystemSelect2 = function(){
		//控件类型
		this.widgetType = 'System_Select2';
		...
		this.customOnReady = function(){
			...
			$('#isShowUserName').combobox({value : $('#isShowUserName').val()});
			if ($('#required').val() != '') {
		...
		this.createWidget = function(data){
			var $systemSelect2 = $('<select><option value="-1">请选择...</option></select>');
			$systemSelect2.attr('id',data.id).attr('widgetType',data.widgetType).attr('style','width:150px');;
			return $systemSelect2;
		};
	...
	//注册此控件类型
	ISV.pf.fd.base.FormWidgetFactory.register('System_Select2',w.SystemSelect2);
})(ISV.pf.fd.widget);