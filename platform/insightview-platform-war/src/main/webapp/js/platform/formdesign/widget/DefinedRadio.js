...
(function(w){
	w.DefinedRadio = function(){
		//控件类型
		this.widgetType = 'DefinedRadio';
		...
		this.createWidget = function(data){
			...
			for(var i=0; i<customInitValues.length; i++){
				sexRadio += inputRadio + '<label>' + customInitValues[i].text + '</label>';
			}
			$definedRadio = $(sexRadio);
			return $definedRadio;
		};
	};
	
	//注册此控件类型
	ISV.pf.fd.base.FormWidgetFactory.register('DefinedRadio',w.DefinedRadio);
})(ISV.pf.fd.widget);