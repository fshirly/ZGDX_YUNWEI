  ...
  <body>
	...
	<table id="editBiangRecord" class="formtable">
	   <tr>
	      ...
	      <td >
	      <input  id="ipt_biangetitle" name="biangetitle" value="${ProjectContractChange.title}" validator="{'default':'*','length':'1-25'}" msg="{reg:'只能输入1-25位字符！'}"/><dfn>*</dfn>
 	      </td>
 	      <td class="title">变更时间：</td>
 	      <td >
 	      <input type="hidden" value="${singtime }" id="singtime"/>
 	      <input  id="ipt_biangshij" name="biangshij"  class="easyui-datebox" value="${changeTime_temp}"  validator="{'default':'*','type':'datetimebox'}" data-options="editable: false"/><dfn>*</dfn></td>
 	   </tr>
 	   <tr>
 	     <td class="title">变更内容：</td>
 	     <td colspan="3"><textarea id="ipt_bgnr" class="x2" rows="3" validator="{'default':'*','length':'0-255'}" msg="{reg:'只能输入1-255位字符！'}">${ProjectContractChange.description}</textarea></td>
 	   </tr>
 	   <tr>
 	     <td class="title">确认人：</td>
 	      <td colspan="3"><input id="qrr" value="${ProjectContractChange.confirmManName}" validator="{'default':'*','length':'1-10'}" msg="{reg:'只能输入1-10位字符！'}"/><dfn>*</dfn></td>
 	   </tr>
 	   <tr>
 	     <td class="title">相关附件：</td>
 	     <td colspan="3">
 	     <input type="file" id="xgfjcontractfilechange" value="${ProjectContractChange.url}"/>
             </td>
 	   </tr>
 	 </table>
 	  <div class="conditionsBtn">
		 <a href="javascript:void(0);" id="btnContractchaneupdate" class="fltrt">确定</a>
   ...
   platform.contractManager.contractChaneUpdate = (function(){
    f('#btnContractchaneupdate').click(contractChaneInfoUpdate);
    var upload_path='';
    var id="${ProjectContractChange.id}";//
    $(function(){
    	 /*
         *上传变更附件
         */
  	 $('#xgfjcontractfilechange').f_fileupload({
			...
			imgHeight:10000,
			downloadFile:'bb',
			upLoadBtnId:'editcontractchangefile',
			repeatUpload : true,
			filePreview:true,
    ...
    function contractChaneInfoUpdate(){
    	if (checkInfo('#editBiangRecord')) {
    		...
    		var uri = path + "/contractmanager/contractChaneInfo_update";
    		var ajax_param = {
    			url : uri,
    			type : "post",
    			datdType : "json",
    			...
    			success : function(data) {
    				if (true == data || "true" == data) {
    					...
    					reloadTableCommon('edit_contract_biange_Record');
    					$.messager.alert("提示", "合同变更信息修改失败！", "error");
