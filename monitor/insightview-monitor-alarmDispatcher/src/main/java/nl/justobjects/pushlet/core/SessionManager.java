...
//		System.out.println(" add user"+anEvent.getField("sessionId"));
//        return Session.create(anEvent.getField("sessionId"),anEvent);
//        return Session.create(anEvent);
	...
	public void addSession(Session session) {
		...
		log.info(session.getId() + " at " + session.getAddress() + " added ");
	}
	...
	public Session removeSession(Session aSession) {
		synchronized (mutex) {
			...
			if (session != null) {
				log.info(session.getId() + " at " + session.getAddress() + " removed ");
			}
			sessionCacheDirty = true;
			return session;
		}
	...
	public void start() throws PushletException {
		...
		log.info("started; interval=" + TIMER_INTERVAL_MILLIS + "ms");
	}
	...
	protected void info(String s) {
		log.info("2SessionManager: " + new Date() + " " + s);
	}
	...
	private class AgingTimerTask extends TimerTask {
		...
		public void visit(Session aSession) {
			try {
				// Age the lease
				aSession.age(delta);
				log.debug("AgingTimerTask: visit: " + aSession);

				// Stop session if lease expired
				if (aSession.isExpired()) {
					log.info("AgingTimerTask: Session expired: " + aSession);
					aSession.stop();
				}
			} catch (Throwable t) {
				log.warn("AgingTimerTask: Error in timer task : " + t);
			}
		}
	}
}
