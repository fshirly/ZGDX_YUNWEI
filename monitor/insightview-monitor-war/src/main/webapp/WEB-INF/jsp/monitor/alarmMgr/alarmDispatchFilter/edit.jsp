  ...
  <body>
	...
	<div id="resCiAllInfoDiv" class="easyui-tabs"  data-options="tabPosition:'left',fit:true,plain:false">
		...
		<div title="派单规则 " data-options="iconCls:'icon-info',closable:false" 	style="overflow: hidden;">
					...
					<td ><input type="text" id="cfgName" name="cfgName"  value="${name}" maxlength="64"/><b>*</b></td>
					</tr>
					<tr>
					<td class="title">是否启用：</td>
					<td >
						<label>
						 <input type="radio" name="userDefault" value="1" onclick="$.messager.alert('提示','如果存在已启用的自动派单设置，启动该设置，则已启动的设置将会被禁用。')"
    						<c:if test="${isDefault==1}">  
    						checked="checked"  
   						 	</c:if>  
						  />是
						 <input type="radio" name="userDefault" value="0"
					...
					<td >
						<textarea rows="3" id="descr" name="descr" maxlength="128">${descr}</textarea>
					</td>
					</tr>
					</table>
		</div>
