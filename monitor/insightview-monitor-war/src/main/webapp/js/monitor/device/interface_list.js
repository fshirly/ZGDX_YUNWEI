...
function doInitTable() {
	...
	var uri = path + '/monitor/deviceManager/listInterface?mOClassID=' + mOClassID;
	$('#tblDataList')
			.datagrid(
					{
						width : 'auto',
...
function showBieming(){
	var rows=$('#tblDataList').datagrid('getSelections');
	var moid=rows[0].moID;
	$('#moAlias').val(rows[0].moAlias);
	if(moid!=null && moid!=''){
		$("#winAlaias").window('open');
	}else{
...
function toMoAliasAdd(){
	if(checkInfo("#addTableMotails")){
		var rows=$('#tblDataList').datagrid('getSelections');
		var moid=rows[0].moID;
		var path = getRootPatch();
		var uri=path+"/monitor/deviceManager/updateMoAlias"
		var ajax_param = {
				url : uri,
				type : "post",
				datdType : "json",
				...
				success : function(data) {
					 if(data==true || data=='true'){
						 $("#winAlaias").window('close');
						 reloadTable();
						 $.messager.alert("提示","更新接口别名成功！","info");
					 }else{
						 $.messager.alert("提示","更新接口别名失败！","error");
					 }
				}
			};
			ajax_(ajax_param);
