...
$(document).ready(function() {
	...
	if( moClassID=="7,8,9,138" || moClassID=="7" || moClassID=="8" || moClassID=="9" || moClassID=="75" || moClassID=="138"){
...
function doInitTable() {
	...
	var uri = path + '/monitor/deviceManager/listDevice?classID=' + mOClassID;
	$('#tblDataList')
			.datagrid(
					{
						width : 'auto',
	...
	var ajax_param = {
		...
		success : function(data) {
			if(data == true){
				...
				if(moClassName=="MobileAppManger"){
					moClass="mobileappmanger";
					title = '移动应用管理'+moName+'视图';
				}
				if(moClassName=="PVM"){
					moClass="pvm";
					title = '虚拟机'+moName+'视图';
				}
				if(moClassName=="PHost"){
					moClass="PHost";
					title = '物理主机'+moName+'视图';
				}
                var urlParams = "?moID=" + moid + "&moClass=" + moClass + "&flag=device";
                var uri = path + "/monitor/gridster/showPortalView" + urlParams;
