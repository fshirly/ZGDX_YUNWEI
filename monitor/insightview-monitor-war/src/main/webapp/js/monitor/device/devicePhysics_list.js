...
function doInitTable() {
	...
	var uri = path + '/monitor/deviceManager/listPhysics?classID='+classID;
	$('#tblDataList')
			.datagrid(
					{
						width : 'auto',
...
//						 alert("sort:"+sort+",order："+order+"");
						...
						columns : [ [
								...
								{  
									...
									sortable:true,
									formatter : function(value, row, index) {
						        			if(row.moClassId != 0 && (flag ==null || flag =="") && row.nemanufacturerid != 0){
						        				var to = "&quot;" + row.moid
												+ "&quot;,&quot;" + value
												+ "&quot;"
...
//			viewDevicePortal(moid,data);
			if(data=="SwitcherL2" ||data=="SwitcherL3"){
				data="Switcher";
			}
...
function doDelePhyObjects(){
	...
	if(ip !=null){
		...
		if(null !=object && object !=""){
			...
			var sb = "<div id='phydlg'><div class='conditionsBtn'>"
				+"<a href='javascript:toAffirm(\""+ids+"\",\""+newids+"\");'  id='btnSave'>确定</a>"
			+"<a href='javascript:void(0);' onclick='javascript:closePhyanle()' id='btnBack'>取消</a></div></div>";
			var phydlg = $(sb).appendTo(document.body).dialog({
				title:'提示',
				width: 450,
				height: 500,
				content: '',
				...
				closed: true,
				onClose:function(){
				$('#phydlg').panel('destroy');
				}
			});
		var serviceTab =$('<table id="serviceData"></table>').prependTo(phydlg.dialog("body")).datagrid({
			    ...
			    columns:[[  
					...
					{field:'serviceName',title:'服务名称', width:70},  
			        {field:'type',title:'类型', width:100},    
			        {field:'ip',title:'设备IP/设备名称', width:200,
			        	formatter:function(value,row,index){ 
			        	var val ;
						var t = row.ip;
						if(t=="no IP" || t=="no ip"){
 ...
 * @return
 */
function closePhyanle(){
	$('#phydlg').panel('close');
	$('#phydlg').panel('destroy');
}
function closephydlgTask(){
	$('#phydlgTask').panel('close');
	$('#phydlgTask').panel('destroy');
}
/**
 * 确认删除该设备是否有采集任务
 * @return
 */
	...
	if( null !=object && object!=""){
		var dlgsb = "<div id='phydlgTask'><div class='conditionsBtn'>"
			+"<a href='javascript:void(0);' onclick='javascript:closephydlgTask()' id='btntaskSave'>关闭</a></div></div>";
		var phydlgTask = $(dlgsb).appendTo(document.body).dialog({
			title:'提示',
			width: 400,
			height: 300,
			content: '',
	...
	var serviceTab =$('<table id="taskData"></table>').prependTo(phydlgTask.dialog("body")).datagrid({
