$(document).ready(function(){
    ...
    if (flag == "null" || flag == "" || flag == null) {
    	doInitTable();
        $('#tblMsServerList').datagrid('hideColumn', 'dbmsMoid');
    }else{
		doInitChooseTable();
	}
});
...
function doInitTable(){
    ...
    var uri = path + '/monitor/msDbManage/listMsServerInfos?dbmsMoId='+dbmsMoId;
    $('#tblMsServerList').datagrid({
        width: 'auto',
        height: 'auto',
        nowrap: false,
        striped: true,
    ...
    $(window).resize(function(){
        $('#tblMsServerList').resizeDataGrid(0, 0, 0, 0);
    });
}
...
function reloadTable(){
    $('#tblMsServerList').datagrid('options').queryParams = {
        "serverName": $("#serverName").val(),
        "ip": $("#ip").val()
    };
    reloadTableCommon_1('tblMsServerList');
}
...
function viewDevicePortal(moid, dbmstype, moName){
    var title = 'SqlServer ' + moName + '视图';
    var moClass = dbmstype;
    var moClassName = "MsSqlServer";
...
function sel(moId,dbmsMoid){
    if (window.opener) {
        ...
        if (flag == "1") {
            ...
            window.opener.findMsServerInfo();
            window.close();
        }
        else {
            fWindowText2 = window.opener.document.getElementById("ipt_msServerMoId");
            fWindowText2.value = moId;
            window.opener.findMsSqlServerInfo();
            window.close();
        }
    }
}
