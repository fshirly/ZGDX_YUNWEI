...
// alert("sort:"+sort+",order："+order+"");
						...
						},
						{
							...
							'iconCls' : 'icon-hand',
							handler : function() {
								alarmDistribute.toBathSendSingle();
							}
						},
						{
							'id' : 'alarmCount',
						...
						onLoadSuccess:function(){
							$('#alarmCount').children().children().attr("style", "font-weight:bold");
							var path = getRootName();
							var uri = path + "/monitor/alarmActive/listRoomAlarmCount";
							var ajax_param = {
								url : uri,
								type : "post",
								datdType : "json",
								...
								success : function(data) {
									$('#emergencyBut').children().children().text(data.emergency);
									$('#severityBut').children().children().text(data.severity);
									$('#ordinaryBut').children().children().text(data.ordinary);
									$('#promptBut').children().children().text(data.prompt);
									$('#uncertainBut').children().children().text(data.uncertain);
								}
							};
							ajax_(ajax_param);
							}
 ...
 * @return
 */
function toBathConfirmsAlarmCount(){
	var ids = null;
	...
	if (null != ids) {
		if (null == flag){
	        ...
	        collapsible:false,
	        modal:true,
	        href: getRootName() + '/monitor/alarmActive/toBathAlarmActiveConfirm?id='+ids + "&flag=statis"
	     });
		}else{
...
function toBathCalcel() {
	...
	if (null != ids) {
		...
		$.messager.confirm("提示", "确定要取消确认告警？", function(r) {
			if (r == true) {			
				var uri =getRootName()+"/monitor/alarmActive/bathCancelAlarmActiveConfirm";
    			var ajax_param = {
    				url : uri,
    				type : "post",
    				datdType : "json",
 ...
 * @return
 */
function toBathClearssAlarmCount(){
	var ids = null;
	var rows  = $('#tblDataList').datagrid('getChecked');
	var flag = null;
	for(var i=0; i<rows.length; i++){
	...
	if (null != ids) {
        ...
        collapsible:false,
        modal:true,
        href: getRootName() + '/monitor/alarmActive/toBathClearAlarmActive?id='+ids + "&flag=statis"
		});
		// }else{
...
function toBathDel() {
	...
	if (null != ids) {		
		// if (null == flag){
			...
			if (r == true) {			
				var uri =getRootName()+"/monitor/alarmActive/bathDeleteAlarmActive";
    			var ajax_param = {
    				url : uri,
    				type : "post",
    				datdType : "json",
...
function toView(id){
	parent.parent.$('#popWin').window({
        ...
        collapsible:false,
        modal:true,
        href: getRootName() + '/monitor/alarmActive/toAlarmActiveDetail?alarmID='+id
     });
}
...
function toConfirm(id){
	        ...
	        collapsible:false,
	        modal:true,
	        href: getRootName() + '/monitor/alarmActive/toAlarmActiveConfirm?alarmID='+id
	     });
}
...
function toCalcel(id) {
		$.messager.confirm("提示", "确定要取消确认告警？", function(r) {
			if (r == true) {			
				var uri =getRootName()+"/monitor/alarmActive/cancelAlarmActiveConfirm";
    			var ajax_param = {
    				url : uri,
    				type : "post",
    				datdType : "json",
...
function toClear(id){
        ...
        collapsible:false,
        modal:true,
        href: getRootName() + '/monitor/alarmActive/toClearAlarmActive?alarmID='+id
		});
}
...
// 告警派发功能对象
alarmDistribute = {};
alarmDistribute.selectPerson = function(){
	var names = [];
	var ids = [];
...
alarmDistribute.toBathSendSingle = function(){
	var ids = null;
	var rows  = $('#tblDataList').datagrid('getChecked');
	/*告警派单二期匹配*/
	for (var i = 0; i < rows.length; i++) {
