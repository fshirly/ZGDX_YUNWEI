...
//						fit : true,// 自动大小
    ...
    $(window).resize(function() {
        $('#tblNotifyToUser').resizeDataGrid(0, 0, 0, 0);
    });
}
/*
 * 更新通知策略通知人表数据
 ...
 * 验证规则名称是否存在
 */
function checkNamebeforeAdd(){
	var path = getRootName();
	var uri = "";
	var policyID = $("#policyID").val();
	if(policyID==""||null==policyID){
		uri = path + "/monitor/alarmNotifyCfg/checkNamebeforeAdd";
	}else{
		uri = path + "/monitor/alarmNotifyCfg/checkNamebeforeUpdate";
	}
	var ajax_param = {
			url : uri,
			type : "post",
...
function checkEmailAlarmContent(){
	...
	var clearlength = emailClearAlarmContent.length;
	var deletelength = emailDeleteAlarmContent.length;
	if(length>2000){
		$.messager.alert("提示", "邮件模板内容过长，请修改！", "error");
		return false;
	}else if(clearlength>2000){
		$.messager.alert("提示", "清除告警邮件模板内容过长，请修改！", "error");
		return false;
	}else if(deletelength>2000){
		$.messager.alert("提示", "删除告警邮件模板内容过长，请修改！", "error");
		return false;
	}else{
		return true;
	}
...
function doAddAlarmNotifyCfg(){
	...
	var ajax_param = {
		...
		"notifyCfg.smsAlarmContent":$("#ipt_smsAlarmContent").val(),
		"notifyCfg.smsClearAlarmContent":$("#ipt_smsClearAlarmContent").val(),
		"notifyCfg.smsDeleteAlarmContent":$("#ipt_smsDeleteAlarmContent").val(),
		...
		success:function(data){
			if (true == data || "true" == data) {
				$.messager.alert("提示", "告警通知策略新增成功！", "info");
				$('#popWin').window('close');
				window.frames['component_2'].reloadTable();
			} else{
				$.messager.alert("提示", "告警通知策略新增失败！", "error");
			}
		}
	};	
	if(result == true){
...
function doMainAdd(exeMethod){
	...
	var isPhone = $('input[name="isPhone"]:checked').val();
	var path = getRootName();
	var uri=path+"/monitor/alarmNotifyCfg/addAlarmNotifyCfg";
	var ajax_param = {
			url : uri,
			type : "post",
			datdType : "json",
			data : {
		"policyName" :$("#ipt_policyName").val(),
		"alarmOnCount" :$("#ipt_alarmOnCount").val(),
		"isSms": $('input[name="isSms"]:checked').val(),
		"smsAlarmContent":$("#ipt_smsAlarmContent").val(),
		"smsClearAlarmContent":$("#ipt_smsClearAlarmContent").val(),
		"smsDeleteAlarmContent":$("#ipt_smsDeleteAlarmContent").val(),
		"isEmail":$('input[name="isEmail"]:checked').val(),
		"emailAlarmContent":emailAlarmContent,
		"emailClearAlarmContent":emailClearAlarmContent,
		"emailDeleteAlarmContent":emailDeleteAlarmContent,
		"maxTimes":$("#ipt_maxTimes").val(),
		"isPhone":$('input[name="isPhone"]:checked').val(),
		"voiceMessageType":$('input[name="voiceMessageType"]:checked').val(),
		"phoneAlarmContent":$("#ipt_phoneAlarmContent").val(),
		"phoneClearAlarmContent":$("#ipt_phoneClearAlarmContent").val(),
		"phoneDeleteAlarmContent":$("#ipt_phoneDeleteAlarmContent").val(),
		"alarmVoiceID":$("#ipt_alarmVoiceID").combobox("getValue"),
		"clearAlarmVoiceID":$("#ipt_clearAlarmVoiceID").combobox("getValue"),
		"deleteAlarmVoiceID":$("#ipt_deleteAlarmVoiceID").combobox("getValue"),
		"descr":$("#ipt_descr").val(),
		"t" : Math.random()
		},
		error : function(){
	...
	if(result == true){
		...
		$.messager.alert("提示", "请先完整填写通知策略定义信息！", "error");
	}
}
...
//						fit : true,// 自动大小
    ...
    $(window).resize(function() {
        $('#viewNotifyFilter').resizeDataGrid(0, 0, 0, 0);
    });
}
