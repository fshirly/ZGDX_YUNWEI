...
function doInitTable() {
			...
			.datagrid(
					{
						...
						fit : true,// 自动大小
						fitColumns:true,
						url : path + '/monitor/webSite/listWebSiteInfo?moClassID='+$("#mOClassID").val(),
						// sortName: 'code',
						// sortOrder: 'desc',
						remoteSort : false,
						idField : 'moID',
    ...
    $(window).resize(function() {
        $('#tblWebSiteList').resizeDataGrid(0, 0, 0, 0);
    });
}
			...
			.datagrid(
					{
						...
						fit : true,// 自动大小
						fitColumns:true,
						url : path + '/monitor/webSite/listWebSiteInfo?includeType='+includeType,
						// sortName: 'code',
						// sortOrder: 'desc',
						remoteSort : false,
						idField : 'moID',
						...
						columns : [ [
						        {
								    ...
								    align : 'center',
									formatter : function(value, row, index) {
										return '<a style="cursor: pointer;" onclick="javascript:sel('+ value+','+row.siteType+');">选择</a>';
									}
						        },
						        {
									field : 'available',
									...
									if(value=="1" || value=="11"){
										t="up.png";
										tipValue = "UP";
									}else if(value=="2" || value=="22" || value=="12" || value=="21"){
										t="down.png";
										tipValue = "DOWN";
									}
...
function reloadTable() {
	...
	$('#tblWebSiteList').datagrid('options').queryParams = {
		"siteName" : $("#txtSiteName").val(),
		"siteType" : type
	};
	reloadTableCommon_1('tblWebSiteList');
}
...
function sel(moId,siteType){
	if(window.opener) { 
	     fWindowText1 = window.opener.document.getElementById("ipt_webSiteMoID"); 
	     fWindowText1.value = moId; 
	     fWindowText2 = window.opener.document.getElementById("ipt_siteType"); 
	     fWindowText2.value = siteType; 
	     window.opener.findSiteInfo();
	     window.close();
	} 
...
function doDeleWebSiteObjects(){
	var ids=null;
	
	var rows  = $('#tblWebSiteList').datagrid('getChecked');
	for(var i =0;i<rows.length;i++){
		if (null == ids) {
			ids = rows[i].moID+"_"+rows[i].siteType;
		} else {
			ids += "," + rows[i].moID+"_"+rows[i].siteType;
		}
	}
 ...
 */
function showCMDB(){
	var rows  = $('#tblWebSiteList').datagrid('getSelections');
...
function closeDlgweb(){
	$('#dlgWeb').panel('close');
	$('#dlgWeb').panel('destroy');
}
/**
 * 确认删除该设备是否有采集任务
 * @return
...
function toAffirmWebSite(ids){
	...
	if( null !=object && object !=""){
		var dlgsb = "<div id='dlgWeb'><div class='conditionsBtn'>"
			+"<a href='javascript:void(0);' onclick='javascript:closeDlgweb()' id='btntaskSave'>关闭</a></div></div>";
	...
	var serviceTab =$('<table id="webData"></table>').prependTo(dlgTask.dialog("body")).datagrid({
		...
		$('#webData').datagrid('loadData',object);
		dlgTask.dialog("open");
		return ;
	}
