...
function doInitTable() {
			...
			.datagrid(
					{
						...
						fit : true,// 自动大小
						fitColumns:true,
						url : path + '/monitor/webSite/listWebSiteDnsInfo',
						// sortName: 'code',
						// sortOrder: 'desc',
						remoteSort : false,
						idField : 'moID',
    ...
    $(window).resize(function() {
        $('#tblWebSiteDnsList').resizeDataGrid(0, 0, 0, 0);
    });
}
...
function reloadTable() {
	$('#tblWebSiteDnsList').datagrid('options').queryParams = {
		"siteName" : $("#txtSiteName").val()
	};
	reloadTableCommon_1('tblWebSiteDnsList');
}
...
function doDeleDNS(){
	...
	var rows  = $('#tblWebSiteDnsList').datagrid('getChecked');
	for(var i =0;i<rows.length;i++){
		if (null == ids) {
			ids = rows[i].moID+"_"+2;// 2代表DNS
		} else {
			ids += "," + rows[i].moID+"_"+2;
		}
	}
 ...
 */
function showCMDB(){
	var rows  = $('#tblWebSiteDnsList').datagrid('getSelections');
...
function closeDlgDNS(){
	$('#dlgDNS').panel('close');
	$('#dlgDNS').panel('destroy');
}
/**
 * 确认删除该设备是否有采集任务
 * @return
...
function toAffirmWebSite(ids){
	...
	if(null !=object && object !=""){
		var dlgsb = "<div id='dlgDNS'><div class='conditionsBtn'>"
			+"<a href='javascript:void(0);' onclick='javascript:closeDlgDNS()' id='btntaskSave'>关闭</a></div></div>";
	...
	var serviceTab =$('<table id="dnsData"></table>').prependTo(dlgTask.dialog("body")).datagrid({
		...
		$('#dnsData').datagrid('loadData',object);
		dlgTask.dialog("open");
		return ;
	}
