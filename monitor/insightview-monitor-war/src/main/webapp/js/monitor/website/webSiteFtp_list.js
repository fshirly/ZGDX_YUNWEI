...
function doInitTable() {
			...
			.datagrid(
					{
						...
						fit : true,// 自动大小
						fitColumns:true,
						url : path + '/monitor/webSite/listWebSiteFtpInfo',
						// sortName: 'code',
						// sortOrder: 'desc',
						remoteSort : false,
						idField : 'moID',
    ...
    $(window).resize(function() {
        $('#tblWebSiteListFtp').resizeDataGrid(0, 0, 0, 0);
    });
}
						...
						columns : [ [
						        ...
						        },
						        {
									...
									if(value=="11"){
										t="up.png";
										tipValue = "UP";
									}else if(value=="22" || value=="12" || value=="21"){
										t="down.png";
										tipValue = "DOWN";
									}
...
function reloadTable() {
	$('#tblWebSiteListFtp').datagrid('options').queryParams = {
		"siteName" : $("#txtSiteName").val()
	};
	reloadTableCommon_1('tblWebSiteListFtp');
}
...
function doDeleFTP(){
	...
	var rows  = $('#tblWebSiteListFtp').datagrid('getChecked');
	for(var i =0;i<rows.length;i++){
		if (null == ids) {
			ids = rows[i].moID+"_"+1;// 1代表FTP
		} else {
			ids += "," + rows[i].moID+"_"+1;
		}
	}
 ...
 */
function showCMDB(){
	var rows  = $('#tblWebSiteListFtp').datagrid('getSelections');
...
function closeDlgFTP(){
	$('#dlgFTP').panel('close');
	$('#dlgFTP').panel('destroy');
}
/**
 * 确认删除该设备是否有采集任务
 * @return
...
function toAffirmWebSite(ids){
	...
	if(null !=object && object !=""){
		var dlgsb = "<div id='dlgFTP'><div class='conditionsBtn'>"
			+"<a href='javascript:void(0);' onclick='javascript:closeDlgFTP()' id='btntaskSave'>关闭</a></div></div>";
	...
	var serviceTab =$('<table id="FTPData"></table>').prependTo(dlgTask.dialog("body")).datagrid({
		...
		$('#FTPData').datagrid('loadData',object);
		dlgTask.dialog("open");
		return ;
	}
