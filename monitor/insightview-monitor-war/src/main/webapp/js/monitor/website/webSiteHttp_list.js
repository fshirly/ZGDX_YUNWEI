...
function doInitTable() {
			...
			.datagrid(
					{
						...
						fit : true,// 自动大小
						fitColumns:true,
						url : path + '/monitor/webSite/listWebSiteHttpInfo',
						// sortName: 'code',
						// sortOrder: 'desc',
						remoteSort : false,
						idField : 'moID',
    ...
    $(window).resize(function() {
        $('#tblWebSiteHttpList').resizeDataGrid(0, 0, 0, 0);
    });
}
...
function reloadTable() {
	$('#tblWebSiteHttpList').datagrid('options').queryParams = {
		"siteName" : $("#txtSiteName").val()
	};
	reloadTableCommon_1('tblWebSiteHttpList');
}
...
function sel(moId){
	if(window.opener) { 
	     ...
	     window.opener.findSiteHttpInfo();
	     window.close();
	} 
...
function doDeleHTTP(){
	...
	var rows  = $('#tblWebSiteHttpList').datagrid('getChecked');
	for(var i =0;i<rows.length;i++){
		if (null == ids) {
			ids = rows[i].moID+"_"+3;// 3代表HTTP
		} else {
			ids += "," + rows[i].moID+"_"+3;
		}
	}
 ...
 */
function showCMDB(){
	var rows  = $('#tblWebSiteHttpList').datagrid('getSelections');
...
function closeDlgHTTP(){
	$('#dlgHTTP').panel('close');
	$('#dlgHTTP').panel('destroy');
}
/**
 * 确认删除该设备是否有采集任务
 * @return
...
function toAffirmWebSite(ids){
	...
	if(null !=object && object !=""){
		var dlgsb = "<div id='dlgHTTP'><div class='conditionsBtn'>"
			+"<a href='javascript:void(0);' onclick='javascript:closeDlgHTTP()' id='btntaskSave'>关闭</a></div></div>";
	...
	var serviceTab =$('<table id="HTTPData"></table>').prependTo(dlgTask.dialog("body")).datagrid({
		...
		$('#HTTPData').datagrid('loadData',object);
		dlgTask.dialog("open");
		return ;
	}
