			...
			.datagrid(
					{
						...
						fit : true,// 自动大小
						fitColumns:true,
//						url : path + '/monitor/hostManage/getSnapshotInfo?type=all&moClass='+moClass,
						url : path + '/monitor/hostManage/querySnapshotInfo?type=all&moClass='+moClass,
						idField : 'fldId',
						columns : [ [
								{
									field : 'classlable',
									...
									/*	if(row.moClassID == 5 ||row.moClassID == 6){
											return "<a href='javascript:void(0)' onclick='showRouterOrSwitherView("+row.moClassID+")'"
											+ ">"
											+ value + "</a>";
										}else if(row.moClassID == 7 || row.moClassID == 8|| row.moClassID == 133 ||
												row.moClassID == 118 ||row.moClassID == 135 ||row.moClassID == 132 ||
												row.moClassID == 138 || row.moClassID == 136
										 ){
...
function toShowAlarmList(moclassid){
	var url=getRootName() +'/monitor/alarmActive/toAlarmActiveList?moClassID='+moclassid+'&host=allSnapshost';
...
//预览视图
function showRouterOrSwitherView(moclassid){
...
function toShowTypeList(moclassid){
	...
	if(moclassid=="7"){
		...
		title = '虚拟宿主机列表';
	}else if(moclassid=="133"){
		title = '移动应用代理设备列表';
	}else if(moclassid=="118"){
		title = 'VPN设备列表';
	}else if(moclassid=="135"){
		title = '代理网关设备列表';
	}else if(moclassid=="132"){
		title = '移动应用管理设备列表';
	}else if(moclassid=="138"){
		title = '虚拟机列表';
	}else if(moclassid=="136"){
		title = '探针设备列表';
	}
	var isExistTabs = parent.parent.document.getElementById("tabs_window");
	var isPartentTabs = parent.document.getElementById("tabs_window");
