$(document).ready(function() {
	 doSnapshotInitTables();//设备快照
	  doHostCPUInitTables();// 主机CPU 
	  doHostMemoryInitTables();// 主机内存
	  doVirtualMemoryInitTables();// 虚拟内存
	  doHostDiskInitTables();// 主机硬盘
	  doFlowsInitTables();// 接口流量
	 	// doInitTables();
	doAlarmInitTables();// 最新告警
	doIfUsageInitTables();// 接口带宽使用率

});

//设备快照
function doSnapshotInitTables() {
							...
							.datagrid(
									{
										...
										columns : [ [
												...
												},
												{
													...
													align : 'center',
													formatter : function(value, row, index) {
												          return "<font color='red'>"+row.alarmcount+"</font>";
												         }
												},
												{
													field : 'perfvalue',
													...
													align : 'center',
													 formatter : function(value, row, index) {
											          return "<font color='red'>"+row.devicealarmcount+"</font>/"+row.devicecount;
											         }
											        }
											        ] ]
											     });
							...
							.datagrid(
									{
										...
										collapsible : false,// 是否可折叠的
										fit : true,// 自动大小
										url : path + '/monitor/hostManage/topPerfCpuInfo?tableName=PerfSnapshotHost',
										...
										columns : [ [
												...
												},
												{
													...
													align : 'center',
													formatter : function(value, row, index) {
												          return "<a href='/fable/monitor/hostManage/toShowHostDetail?deviceIP="+value+"' target='_blank'>"+value+"</a>";
												         }
												},
												{
													field : 'perfvalue',
													...
													align : 'center',
													 formatter : function(value, row, index) {
											          var color = value < 90 ? '#00ff00' : '#ff0000';// #FFFF00
