...
function initRuleInfo(){
    ...
    var uri = path + "/monitor/syslogRule/initRuleInfo";
    var ajax_param = {
        url: uri,
        type: "post",
        datdType: "json",
        ...
        success: function(data){
            ...
            if (serverIP != "*") {
                $("#trServerIp").show();
                $("input:radio[name='isAll'][value=2]").attr("checked", 'checked');
                $("#deviceips").val(data.serverIP);
                
                var serverIPArray = data.serverIP.split(",");
                var showOptions = "";
                for (var i = 0; i < serverIPArray.length; i++) {
                    showOptions += serverIPArray[i] + "\r\n";
                }
                $("#ipt_serverIP").val(showOptions);
            } else {
                $("#trServerIp").hide();
                $("input:radio[name='isAll'][value=1]").attr("checked", 'checked');
            }
            var facilitylist = data.facilitylist;
            var severitylist = data.severitylist;
            if (facilitylist != "" && facilitylist != null) {
                var facilityArray = facilitylist.split(",");
                for (var i = 0; i < facilityArray.length; i++) {
                    var checkboxs = document.getElementById("ipt_f" + facilityArray[i]);
                    checkboxs.checked = true;
                }
            }
            
            if (severitylist != "" && severitylist != null) {
                var severityArray = severitylist.split(",");
                for (var i = 0; i < severityArray.length; i++) {
                    var checkboxs = document.getElementById("ipt_s" + severityArray[i]);
                    checkboxs.checked = true;
                }
            }
...
//			console.log(keyword)
            if (keyword == "" || keyword == null) {
                document.getElementById("ipt_keyword").className = 'normalInput x2';
				document.getElementById("ipt_keyword").value = "多个关键字用','分隔";
            } else {
                document.getElementById("ipt_keyword").className = 'x2';
            }
        }
    };
    ajax_(ajax_param);
...
function checkSyslogForm(){
    var result = checkInfo("#divModifySyslog");
    if (result) {
...
function doUpdateSyslogRule(){
    ...
    var ruleName = $("#ipt_ruleName").val();
    var path = getRootName();
    var uri = path + "/monitor/syslogRule/doUpdateSyslogRule";
    var ajax_param = {
        url: uri,
        type: "post",
        dataType: "json",
        ...
        success: function(data){
            var updateRuleFlag = data.updateRuleFlag;
            var isHaveTask = data.isHaveTask;
            var updateTaskFlag = data.updateTaskFlag;
            if (updateRuleFlag == true || updateRuleFlag == "true") {
                if (isHaveTask == false || isHaveTask == "false") {
                    $.messager.alert("提示", "syslog监测配置更新成功！", "info");
                } else {
                    if (updateTaskFlag == true || updateTaskFlag == "true") {
                        $.messager.alert("提示", "syslog监测配置更新成功,并且更新syslog任务成功！", "info");
                    } else if (updateTaskFlag == true || updateTaskFlag == "true") {
                        $.messager.alert("错误", "syslog监测配置更新成功,更新syslog任务失败！", "error");
                    }
                }
                $('#popWin').window('close');
                window.frames['component_2'].reloadTable();
            } else {
                $.messager.alert("错误", "syslog监测配置更新失败！", "error");
            }
        }
    };
    ajax_(ajax_param);
