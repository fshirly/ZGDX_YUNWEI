...
function doInitTable(){
    ...
    $('#tblSyslogTask').datagrid({
        ...
        url: path + '/monitor/syslogTask/listSyslogTasks',
        //        idField: 'taskID',
    ...
    $(window).resize(function(){
        $('#tblSyslogTask').resizeDataGrid(0, 0, 0, 0);
    });
}
...
function reloadTable(){
    ...
    $('#tblSyslogTask').datagrid('options').queryParams = {
        "taskID": taskID,
        "ruleName": ruleName,
        "collectorName": collectorName,
    };
    reloadTableCommon('tblSyslogTask');
}
...
function getOpProcess(){
    ...
    for (var i = 0; i < rows; i++) {
        var processValue = $('#tblSyslogTask').datagrid('getRows')[i]['progressStatus'];
        if (processValue < 5) {
            var taskId = $('#tblSyslogTask').datagrid('getRows')[i]['taskID'];
            taskIds += taskId + ":" + i + ",";
        }
    ...
    var ajax_param = {
        ...
        success: function(data){
            ...
            for (var i = 0; i < errorInfoLst.length; i++) {
                ...
                $('#tblSyslogTask').datagrid('updateRow', {
                    index: indexValue,
                    row: {
                        errorInfo: errorInfo
                    }
