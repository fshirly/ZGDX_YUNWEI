...
function doInitTable(){
    ...
    $('#tblOfflineDiscoverTask').datagrid({
        ...
        url: path + '/monitor/offlineDiscover/listOfflineTasks',
        remoteSort: true,
        idField: 'fldId',
        ...
        columns: [[{
            ...
            formatter: function(value, row, index){
                ...
                if (tasktype == 1) {
                    return row.ipaddress1 + "-" + row.ipaddress2;
                }
                else if (tasktype == 2) {
                    return row.ipaddress1 + "-" + row.netmask;
                }
                else if (tasktype == 3) {
                    return row.ipaddress1 + "-" + row.hops;
                }
                else {
                    return row.ipaddress1;
                }
            },
        }, {
            field: 'tasktype',
    ...
    $(window).resize(function(){
        $('#tblOfflineDiscoverTask').resizeDataGrid(0, 0, 0, 0);
    });
}
...
function reloadTable(){
    ...
    $('#tblOfflineDiscoverTask').datagrid('options').queryParams = {
        "tasktype": tasktype,
        "ipaddress1": ipaddress1,
        "collectorName": collectorName,
    };
    
    reloadTableCommon('tblOfflineDiscoverTask');
}
...
function getOpProcess(){
    var grid = $('#tblOfflineDiscoverTask');
    ...
    for (var i = 0; i < rows; i++) {
        var processValue = $('#tblOfflineDiscoverTask').datagrid('getRows')[i]['progressstatus'];
        if (processValue < 5) {
            var taskId = $('#tblOfflineDiscoverTask').datagrid('getRows')[i]['taskid'];
            taskIds += taskId + ":" + i + ",";
        }
    ...
    var ajax_param = {
        ...
        success: function(data){
            ...
            for (var i = 0; i < errorinfoLst.length; i++) {
                ...
                $('#tblOfflineDiscoverTask').datagrid('updateRow', {
                    index: indexValue,
                    row: {
                        errorinfo: errorinfo
                    }
...
function getOpCollector(){
    ...
    var taskIds = '';
    for (var i = 0; i < rows; i++) {
        var collectorName = $('#tblOfflineDiscoverTask').datagrid('getRows')[i]['collectorName'];
...
function resetFormDiv(divId){
	resetForm(divId);
	$("#txtTaskType").combobox("setValue","-1");
}
