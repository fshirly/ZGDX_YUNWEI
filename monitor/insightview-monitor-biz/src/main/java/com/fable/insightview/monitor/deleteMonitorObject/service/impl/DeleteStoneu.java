...
@Service("common_109")
public class DeleteStoneu extends DeleteObjectServiceImpl {
	...
	public void deletePerfAndAlarm(Map<String, Object> map) {
		...
		// 查询子对象的信息
		Map<String, Object> StoneuChildMap = new HashMap<String, Object>();
		StoneuChildMap.put("MOID", MOID);
		List<Integer> childMoid =  deviceMapper.queryStoneuChildMOID(StoneuChildMap);
		for (Integer moid :childMoid) {
			// 删除斯特纽性能数据
			deviceMapper.deletePerfStoneuModevice(moid);
			// 删除斯特纽
			deviceMapper.deleteStoneuModevice(moid);
		}
