...
@Service("common_20")
public class DeleteTomcat extends DeleteObjectServiceImpl{
	...
	public boolean delete(Map<String, Object> map) {
		...
		parm.put("MOID", MOID);
		List<ServiceBean> middleWareList =  middelWareMapper.queryMiddleWareInfo(parm);
	...
	/*	// 删除快照
		...
		// 查询MOMiddleWareJ2eeApp的MOID
		 ...
		 String j2eeMOID = j2eeMOIDList.toString().replace("[", "").replace("]", "");
		 if(j2eeMOID !=null &&  !"".equals(j2eeMOID)){
			 ...
			 List<Integer> webModubleMOIDList =middelWareMapper.queryJ2eeAppMOID(parm);
			 String webModubleMOID = webModubleMOIDList.toString().replace("[", "").replace("]", "");
		...
		childMap.put("MOMiddleClassLoad", MOID);
		childMap.put("MOMiddleWareJVM", MOID);
		childMap.put("MOMiddleWareMemory", MOID);
