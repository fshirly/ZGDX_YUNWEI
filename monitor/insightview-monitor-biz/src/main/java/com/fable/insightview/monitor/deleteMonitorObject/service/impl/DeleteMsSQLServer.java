...
@Service("common_86")
public class DeleteMsSQLServer extends DeleteObjectServiceImpl {
	@Autowired
	...
	public boolean delete(Map<String, Object> map) {
		...
		childMOIDMap.put("MOMsSQLServer", MOID);
		...
		/*
		 * if(moids !=null && !"".equals(moids)){ // 删除告警数据 Map<String, Object>
		 ...
		 * alarmMap.put("MOKPIThreshold", moids); for(String alarmKey :
		 * alarmMap.keySet()){ parm = new HashMap<String, Object>();
		 * parm.put("alarmTableName", alarmKey); parm.put("MOID", moids); int k;
		 * do{ k= dbserviceMapper.deleteAlarmDetail(parm); }while(k>0);
		 * 
		 * } // 删除该设备的性能快照数据 parm = new HashMap<String, Object>();
		 * parm.put("prefTableName", "PerfSnapshotDatabase"); parm.put("MOID",
		 * moids); int m; do{ m=dbserviceMapper.deletePrefSnapshot(parm);
		 * }while(m>0);
		 * 
		 * // 删除任务性能表 Map<String, Object> taskMap = new HashMap<String,
		 * Object>(); taskMap.put("SysPerfPollTask", MOID); for(String taskKey :
		 * taskMap.keySet()){ parm = new HashMap<String, Object>();
		 * parm.put("tableName", taskKey); parm.put("MOID", MOID);
		 * dbserviceMapper.deleteTaskAndvars(parm); }
		 * 
		 * // 删除该设备的子对象 Map<String, Object> PrefMap = new HashMap<String,
		 * Object>(); PrefMap.put("MOMsSQLDatabase", moids);
		 * PrefMap.put("MOMsSQLDevice", moids); for(String prefKey :
		 * PrefMap.keySet()){ parm = new HashMap<String, Object>();
		 * parm.put("tableName", prefKey); parm.put("MOID", moids); int w; do{
		 * w=dbserviceMapper.deleteChildDevice(parm); }while(w>0); } //删除凭证 parm
		 * = new HashMap<String, Object>(); parm.put("snmpTableName",
		 * "SysDBMSCommunity"); parm.put("tableName", "MOMsSQLServer");
		 * parm.put("DBtype", "MsSql"); parm.put("MOID", MOID);
		 * dbserviceMapper.deleteSNMP(parm); // 删除告警通知策略、告警视图定义、自动派单设置
		 ...
		 * alarmFiterMap.put("AlarmDispatchFilter", moids); for(String fiterKey
		 * : alarmFiterMap.keySet()){ parm = new HashMap<String, Object>();
		 * parm.put("tableName", fiterKey); parm.put("MOID", moids); int n; do{
		 * n = dbserviceMapper.deleteAlarmFilter(parm); }while(n>0); } }
		 */
		// 删除凭证
		parm = new HashMap<String, Object>();
		parm.put("snmpTableName", "SysDBMSCommunity");
		parm.put("tableName", "MOMsSQLServer");
		parm.put("DBtype", "MsSql");
		parm.put("MOID", MOID);
	...
	public void deletePerfAndAlarm(Map<String, Object> map) {
		...
		if (moids != null && !"".equals(moids)) {
			...
			PrefMap.put("MOMsSQLDatabase", moids);
			PrefMap.put("MOMsSQLDevice", moids);
